...
<#if product?exists>
    ...
    <div class="productsummary">
        ...
        <div class="productinfo">
          ...
          <#if productContentWrapper.get("BRAND_NAME")?exists && (productContentWrapper.get("BRAND_NAME")?trim)?has_content>
            <div>
              <i>Brand: ${productContentWrapper.get("BRAND_NAME")}</i>
            </div>
          </#if>
          <div>${productContentWrapper.get("DESCRIPTION")?if_exists}
          <#--
          <#if daysToShip?exists>&nbsp;-&nbsp;${uiLabelMap.ProductUsuallyShipsIn} <b>${daysToShip}</b> ${uiLabelMap.CommonDays}!</#if>
          -->
          </div>
