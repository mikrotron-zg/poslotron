...
<#if miniProduct?exists>
    ...
    <ul>
        <li><em>${miniProduct.productId}</em>&nbsp;
          <strong>
      ...
      <li>
        <form method="post" action="<@ofbizUrl>additem<#if requestAttributes._CURRENT_VIEW_?has_content>/${requestAttributes._CURRENT_VIEW_}</#if></@ofbizUrl>" name="${miniProdFormName}" style="margin: 0;">
          <fieldset>
            ...
            <a href="javascript:document.${miniProdFormName}.submit()" class="buttontext"><span style="white-space: nowrap;">${uiLabelMap.OrderAddToCart}</span></a>
          </fieldset>
        </form>
      </li>
    </#if>
