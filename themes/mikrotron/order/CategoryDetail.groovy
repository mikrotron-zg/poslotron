...
andMap.put("useCacheForMembers", false);
...
if(productStore) {
    ...
    productCategoryMembers.each { productCategoryMember ->
        ...
        if (isMarketingPackage) {
            ...
            facilities = delegator.findList("InventoryItem", EntityCondition.makeCondition([productId : productCategoryMember.productId]), null, null, null, false);
            availableInventory = 0.0;
            if (facilities) {
                facilities.each { facility ->
                    //lastInventoryCount = facility.lastInventoryCount;
                    lastInventoryCount = facility.quantityOnHandTotal;
                    if (lastInventoryCount != null) {
                        availableInventory += lastInventoryCount;
                    }
                }
                if (availableInventory > 0) {
                    productCategoryMember.quantity = availableInventory; // this requires andMap.put("useCacheForMembers", false)
                    productsInStock.add(productCategoryMember);
                }
            }
        }
...
parentProductCategoryRollups.each { productCategoryRollup -> 
    ...
    subcategories.add(categoryContentWrapper);
}
